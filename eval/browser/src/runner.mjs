...
import { createFixture, createFixtureFromResponse } from "./fixtures.mjs";
...
function stage(name, detail = "") {
  ...
  process.stderr.write(
    `${new Date().toISOString()} ${name}${detail ? ` ${detail}` : ""}\n`,
  );
}
...
async function setMeasurementPhase(runtimeState, phase) {
  ...
  if (!control.phase_ack_path || !["baseline", "post_render"].includes(phase))
    return;
  const deadline = performance.now() + timeoutMs;
  while (performance.now() < deadline) {
    try {
      const acknowledgement = JSON.parse(
        await readFile(control.phase_ack_path, "utf8"),
      );
      if (
        acknowledgement.phase === phase &&
        acknowledgement.phase_sequence === runtimeState.phase_sequence
      )
        return;
    } catch {
      /* Python has not atomically published the acknowledgement yet. */
    }
    await new Promise((resolve) => setTimeout(resolve, 5));
  }
  throw new Error(
    `process_monitor_failure: Python did not acknowledge ${phase} RSS sample`,
  );
}
...
async function heapSnapshot(session) {
  if (!session)
    return {
      js_heap_used_bytes: unavailable("CDP session unavailable"),
      js_heap_total_bytes: unavailable("CDP session unavailable"),
      embedder_heap_used_bytes: unavailable("CDP does not expose this metric"),
      backing_storage_bytes: unavailable("CDP does not expose this metric"),
    };
  try {
    const heap = await session.send("Runtime.getHeapUsage");
    return {
      js_heap_used_bytes: Number.isFinite(heap.usedSize)
        ? available(heap.usedSize)
        : unavailable("CDP Runtime.getHeapUsage omitted usedSize"),
      js_heap_total_bytes: Number.isFinite(heap.totalSize)
        ? available(heap.totalSize)
        : unavailable("CDP Runtime.getHeapUsage omitted totalSize"),
      embedder_heap_used_bytes: unavailable(
        "CDP Runtime.getHeapUsage does not expose embedder heap usage",
      ),
      backing_storage_bytes: unavailable(
        "CDP Runtime.getHeapUsage does not expose backing storage",
      ),
    };
  } catch (error) {
...
function isGraphRequest(item, apiUrl) {
  return (
    item.url.startsWith(apiUrl) &&
    new URL(item.url).pathname.startsWith("/api/graph/")
  );
}
...
async function runCameraSequence(input) {
  ...
  for (let step = 1; step <= input.drag_steps; step += 1) {
    const point = {
      x: x + (input.drag_dx * step) / input.drag_steps,
      y: y + (input.drag_dy * step) / input.drag_steps,
    };
  ...
  sequence.push({
    type: "wheel",
    delta_x: 0,
    delta_y: input.zoom_in_delta_y,
    delay_ms: input.wheel_delay_ms,
  });
  ...
  sequence.push({
    type: "wheel",
    delta_x: 0,
    delta_y: input.zoom_out_delta_y,
    delay_ms: input.wheel_delay_ms,
  });
  return {
...
try {
  if (!controlPath || !resultPath)
    throw new Error(
      "harness_protocol_failure: expected control and result paths",
    );
  control = JSON.parse(await readFile(controlPath, "utf8"));
  const fixture = control.fixture.response
    ? createFixtureFromResponse(control.fixture)
    : createFixture(control.fixture);
  ...
  if (
    ...
    !Number.isInteger(minimumFrameSampleCount) ||
    !Number.isInteger(input.drag_steps) ||
    input.drag_steps < 1 ||
    ![
      input.drag_dx,
      input.drag_dy,
      input.step_delay_ms,
      input.zoom_in_delta_y,
      input.zoom_out_delta_y,
      input.wheel_delay_ms,
    ].every(Number.isFinite) ||
    input.step_delay_ms < 0 ||
    input.wheel_delay_ms < 0
  )
    throw new Error(
      "invalid_configuration: invalid timing or fixed-input configuration",
    );
  ...
  await bounded("replay_health_node", async () => {
    ...
    if (!response.ok || (await response.json()).api_version !== "1")
      throw new Error("replay API health contract mismatch");
  });
  ...
  stage(
    "chromium_launched",
    `pid=${browserServer.process()?.pid ?? "unavailable"}`,
  );
  ...
  await writeRuntimeState(control.runtime_state_path, runtimeState);
  await writeJson(control.browser_pid_path, {
    browser_pid: runtimeState.browser_pid,
  });
  ...
  page.on("requestfailed", (request) =>
    logs.failures.push({ url: request.url(), failure: request.failure() }),
  );
  ...
  await withTimeout("navigation", timeoutMs, () =>
  ...
  await bounded("replay_health_page", () =>
    page.evaluate(async (apiUrl) => {
      const response = await fetch(`${apiUrl}/health`);
      const payload = await response.json();
      if (!response.ok || payload.api_version !== "1")
        throw new Error("replay health contract mismatch");
    }, replay.apiUrl),
  );
  stage("replay_health_ok");
  stage("view_create_start");
  await bounded("view_create", () =>
    page.evaluate(
      (apiUrl) => window.phyloLensEvaluation?.createView(apiUrl),
      replay.apiUrl,
    ),
  );
  stage("view_created");
  ...
  const result = await bounded("view_load_and_raf", () =>
    page.evaluate(() => window.phyloLensEvaluation?.run()),
  );
  if (!result?.ok)
    throw new Error(
      `client_load_failure: ${result?.error ?? "missing page result"}`,
    );
  stage("load_resolved");
  stage("raf_1");
  stage("raf_2");
  ...
  const heapPostRender = await bounded("heap_post_render", () =>
    heapSnapshot(cdp),
  );
  await setMeasurementPhase(runtimeState, "post_render");

  const graphResponsesBeforeCamera = logs.responses.filter((item) =>
    isGraphRequest(item, replay.apiUrl),
  ).length;
  stage("frame_sampling_start");
  await setMeasurementPhase(runtimeState, "frame_experiment");
  await bounded("frame_sampling_setup", () =>
    page.evaluate(() => window.phyloLensEvaluation?.startFrameSampling()),
  );
  const inputRecord = await bounded("camera_sequence", () =>
    runCameraSequence(input),
  );
  const frameIntervalsMs = await bounded("frame_sampling_finish", () =>
    page.evaluate(() => window.phyloLensEvaluation?.finishFrameSampling()),
  );
  stage("frame_sampling_complete", `samples=${frameIntervalsMs.length}`);
  const graphResponsesAfterCamera = logs.responses.filter((item) =>
    isGraphRequest(item, replay.apiUrl),
  ).length;
  const unexpectedViewportRequests =
    graphResponsesAfterCamera - graphResponsesBeforeCamera;
  const replayContractFailure = replay.accessLog.some(
    (item) => item.status >= 400,
  );
  const resourceTimings = await bounded("resource_timings", () =>
    page.evaluate(() =>
      performance.getEntriesByType("resource").map((entry) => entry.toJSON()),
    ),
  );
  await writeJson(control.frame_samples_path, {
    frame_intervals_ms: frameIntervalsMs,
  });
  await writeJson(control.request_samples_path, {
    requests: logs.requests,
    responses: logs.responses,
    resource_timings: resourceTimings,
    failures: logs.failures,
    unexpected_network: logs.unexpected_network,
  });
  ...
  await bounded("view_dispose", () =>
    page.evaluate(() => window.phyloLensEvaluation?.dispose()),
  );

  const validity = observationValidity({
    unexpectedNetworkCount: logs.unexpected_network.length,
  ...
  const output = {
    ...
    message:
      validity.failure_kind === "replay_contract_failure"
        ? "replay_contract_failure: replay returned an unsupported or malformed response"
        : validity.failure_kind === "none"
          ? null
          : `${validity.failure_kind}: ${validity.failure_kind === "unexpected_viewport_request" ? unexpectedViewportRequests : logs.unexpected_network.length}`,
    ...
    artifacts: {
      screenshot_path: control.screenshot_path,
      frame_samples_path: control.frame_samples_path ?? null,
      request_samples_path: control.request_samples_path ?? null,
      replay_access_log_path: control.replay_access_log_path ?? null,
    },
  };
  await writeFile(resultPath, JSON.stringify(output, null, 2));
  emit(output);
} catch (error) {
  const artifacts = await retainDiagnostics().catch((artifactError) => ({
    diagnostics_error: String(artifactError),
  }));
  const failure = {
    status: error?.code === "STAGE_TIMEOUT" ? "timeout" : "failure",
    failure_kind: classifyFailure(error, lastStage),
    message: String(error),
    last_completed_stage: lastStage,
    elapsed_ms: performance.now() - started,
    partial_timing_data: null,
    diagnostic_artifacts: {
      result_path: resultPath ?? null,
      screenshot_path: control?.screenshot_path ?? null,
      ...artifacts,
    },
  };
  if (resultPath) {
  ...
  await closeAll(
    ...
    (resource, error) =>
      logs.cleanup_failures.push({ resource, error: String(error) }),
  );
