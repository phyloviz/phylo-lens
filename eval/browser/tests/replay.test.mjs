...
test("replay rejects unknown routes and records the response", async () => {
  const replay = await startReplayServer(createFixture({ id: "test", nodeCount: 2, triangleCount: 0, seed: 3, labelsEnabled: false }));
  try {
    const response = await fetch(`${replay.apiUrl}/not-a-route`);
    assert.equal(response.status, 404);
    assert.deepEqual(await response.json(), { detail: "Unsupported replay endpoint." });
...
test("replay rejects malformed prepare requests", async () => {
  ...
  try {
    const response = await fetch(`${replay.apiUrl}/api/graph/prepare`, {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ format: "not-newick" }),
    });
