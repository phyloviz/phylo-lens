...
class PostgresPreparedLayoutStore:
        ...
        with self._connect() as connection:
            with connection.transaction():
                ...
                if node_positions:
                    ...
                    with writer._stage(stage_factory, "persist_layouts.node_positions.clear_existing"):
                        connection.execute(
                            """
    ...
    def load_cluster_layouts(self, dataset_id: str, layout_version: str) -> list[ClusterLayout]:
        with self._connect() as connection:
    ...
    def load_node_positions(self, dataset_id: str, layout_version: str) -> list[NodeLayoutPosition]:
        with self._connect() as connection:
            rows = connection.execute(
                """
    ...
    def _persist_metadata(self, connection, artifacts: PreparedLayoutArtifacts, *, stage_factory=None) -> None:
        ...
        with writer._stage(stage_factory, "persist_artifacts.metadata_schema"):
            execute_many_chunked(
                ...
                """,
                [(dataset_id, layout_version, key, field_type) for key, field_type in public_fields],
            )
        ...
        with writer._stage(stage_factory, "persist_artifacts.node_metadata"):
            execute_many_chunked(
                ...
                """,
                writer.node_metadata_rows(dataset_id, layout_version, render_metadata_json_by_node),
            )
...
def execute_many_chunked(connection, sql: str, rows: Iterable[SQLRow], *, batch_size: int = BULK_INSERT_BATCH_SIZE) -> int:
    total = 0
    batch: list[SQLRow] = []
    for row in rows:
        batch.append(row)
...
def load_metadata_schema(connection, *, dataset_id: str, layout_version: str) -> tuple[MetadataSchemaField, ...]:
    rows = connection.execute(
        """
    ...
    return tuple(MetadataSchemaField(key=row["field_key"], type=row["field_type"]) for row in rows)


def load_metadata_rows(connection, *, table: str, key_column: str, dataset_id: str, layout_version: str, keys: set[str]):
    if not keys:
        return {}
    rows = connection.execute(
        f"""
...
def attach_node_metadata(connection, *, dataset_id: str, layout_version: str, nodes: tuple[ViewportNode, ...]):
    if not nodes:
        return nodes
    ...
    return tuple(
        replace(
            node,
            metadata=cluster_metadata.get(node.cluster_id) if node.is_representative else node_metadata.get(node.node_id),
        )
        if (cluster_metadata.get(node.cluster_id) if node.is_representative else node_metadata.get(node.node_id))
        else node
        for node in nodes
    )


def load_cluster_metadata(connection, *, dataset_id: str, layout_version: str, cluster_ids: set[str]):
    if not cluster_ids:
        return {}
    ...
    if not missing:
        return cached
    computed = compute_cluster_metadata(connection, dataset_id=dataset_id, layout_version=layout_version, cluster_ids=missing)
    if computed:
        execute_many_chunked(
            connection,
            """
            ...
            on conflict(dataset_id, layout_version, cluster_id) do update set
                metadata_json = excluded.metadata_json
            """,
            ((dataset_id, layout_version, cluster_id, json.dumps(metadata)) for cluster_id, metadata in computed.items()),
        )
    return {**cached, **computed}


def compute_cluster_metadata(connection, *, dataset_id: str, layout_version: str, cluster_ids: set[str]):
    schema = tuple((field.key, field.type) for field in load_metadata_schema(connection, dataset_id=dataset_id, layout_version=layout_version))
    if not schema:
        return {}
    ...
    member_ids = {node_id for member_ids in members_by_cluster_id.values() for node_id in member_ids}
    ...
    for cluster_id, member_ids_for_cluster in members_by_cluster_id.items():
        if len(member_ids_for_cluster) == 1:
            ...
            metadata = aggregate_cluster_metadata_by_node_ids(member_ids_for_cluster, metadata_by_node, schema)
        if metadata:
            computed[cluster_id] = metadata
    return computed


def load_cluster_members(connection, *, dataset_id: str, layout_version: str, cluster_ids: set[str]):
    if not cluster_ids:
        return {}
    rows = connection.execute(
        """
...
def read_viewport(connection, threshold_cache, *, dataset_id, layout_version, xmin, xmax, ymin, ymax, max_nodes, lod_level=None, cluster_id=None, focus_node_id=None):
    global_bounds = read_global_node_bounds(connection, dataset_id=dataset_id, layout_version=layout_version)
    if cluster_id:
        ...
        edges = tuple(read_edges_for_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, node_ids=member_ids))
        ...
        enriched = attach_node_metadata(connection, dataset_id=dataset_id, layout_version=layout_version, nodes=nodes)
        return ViewportReadResult(
            ...
            metadata_schema=load_metadata_schema(connection, dataset_id=dataset_id, layout_version=layout_version),
        )

    threshold = threshold_for_lod_level(connection, threshold_cache, dataset_id=dataset_id, layout_version=layout_version, lod_level=lod_level)
    if lod_level == 0 and not has_bounds(xmin, xmax, ymin, ymax):
        ...
        nodes, total_node_count = read_ready_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, max_nodes=max_nodes)
        viewport_node_ids = {node.node_id for node in nodes}
        edges = tuple(read_edges_touching_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, node_ids=viewport_node_ids))
        neighbor_ids = {endpoint for edge in edges for endpoint in (edge.source, edge.target) if endpoint not in viewport_node_ids}
        neighbors = tuple(read_node_positions_by_ids(connection, dataset_id=dataset_id, layout_version=layout_version, node_ids=neighbor_ids, max_nodes=len(neighbor_ids)) if neighbor_ids else ())
        nodes = nodes + neighbors
        present_ids = viewport_node_ids | {node.node_id for node in neighbors}
        edges = tuple(edge for edge in edges if edge.source in present_ids and edge.target in present_ids)
        ...
        total_node_count = count_clusters_for_threshold(connection, dataset_id=dataset_id, layout_version=layout_version, threshold=threshold, xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)
        edges = tuple(read_prepared_edges_for_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, lod_level=lod_level or 0, node_ids={node.node_id for node in nodes}))

    nodes = attach_node_metadata(connection, dataset_id=dataset_id, layout_version=layout_version, nodes=nodes)
    return ViewportReadResult(
        dataset_id=dataset_id,
        layout_version=layout_version,
        nodes=nodes,
...
def bounds_clause(prefix: str, xmin, xmax, ymin, ymax) -> tuple[str, tuple[Any, ...]]:
    if None in (xmin, xmax, ymin, ymax):
        return "", ()
    return f"and {prefix}.x between %s and %s and {prefix}.y between %s and %s", (xmin, xmax, ymin, ymax)


def cluster_bounds_clause(xmin, xmax, ymin, ymax) -> tuple[str, tuple[Any, ...]]:
    if None in (xmin, xmax, ymin, ymax):
        return "", ()
    return "and max_x >= %s and min_x <= %s and max_y >= %s and min_y <= %s", (xmin, xmax, ymin, ymax)
...
def read_global_node_bounds(connection, *, dataset_id, layout_version):
    ...
    if row is None or row["min_x"] is None:
        return None
    return LayoutBounds(min_x=row["min_x"], max_x=row["max_x"], min_y=row["min_y"], max_y=row["max_y"])


def threshold_for_lod_level(connection, threshold_cache, *, dataset_id, layout_version, lod_level):
...
def read_lod_zero_without_bounds(connection, threshold_cache, *, dataset_id, layout_version, threshold, max_nodes):
    if threshold is not None:
        nodes = read_cluster_representatives(connection, dataset_id=dataset_id, layout_version=layout_version, threshold=threshold, xmin=None, xmax=None, ymin=None, ymax=None, max_nodes=max_nodes)
        total = count_clusters_for_threshold(connection, dataset_id=dataset_id, layout_version=layout_version, threshold=threshold, xmin=None, xmax=None, ymin=None, ymax=None)
        if total > 1:
            return nodes, total, tuple(read_prepared_edges_for_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, lod_level=0, node_ids={node.node_id for node in nodes}))
    nodes, total = read_distinct_node_positions(connection, dataset_id=dataset_id, layout_version=layout_version, max_nodes=max_nodes)
    return nodes, total, tuple(read_edges_for_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, node_ids={node.node_id for node in nodes}))


def read_ready_nodes(connection, *, dataset_id, layout_version, xmin, xmax, ymin, ymax, max_nodes):
    ...
    return tuple(viewport_leaf_node(row) for row in rows), int(count_row["total_count"]) if count_row else 0


def read_cluster_member_nodes(connection, *, dataset_id, layout_version, cluster_id, max_nodes, focus_node_id=None):
    ...
    return tuple(viewport_leaf_node(row) for row in rows), int(total_row["total_count"]) if total_row else 0


def read_expansion_meta_edges(connection, threshold_cache, *, dataset_id, layout_version, cluster_id, member_ids):
    if not member_ids:
        return [], []
    row = connection.execute(
        """
    ...
    boundary_rows = connection.execute(
        ...
        """,
        (dataset_id, layout_version, list(sorted(member_ids)), list(sorted(member_ids))),
    ).fetchall()
    boundary = []
    outside_ids = set()
    ...
    neighbor_reps = representatives_for_nodes(connection, dataset_id=dataset_id, layout_version=layout_version, threshold=threshold, node_ids=outside_ids)
    ...
    for inside, outside, distance in boundary:
        ...
        bundled[key] = (min_distance(existing[0], distance), existing[1] + 1) if existing else (distance, 1)
    meta_edges = [
        ViewportEdge(edge_id=f"meta_edge:{inside}:{rep_id}", source=inside, target=rep_id, distance=distance, is_meta=True, bundled_edge_count=count)
...
def representatives_for_nodes(connection, *, dataset_id, layout_version, threshold, node_ids):
    if not node_ids:
        return {}
    rows = connection.execute(
        """
...
def read_cluster_representatives(connection, *, dataset_id, layout_version, threshold, xmin, xmax, ymin, ymax, max_nodes):
...
def count_clusters_for_threshold(connection, *, dataset_id, layout_version, threshold, xmin, xmax, ymin, ymax):
    clause, params = cluster_bounds_clause(xmin, xmax, ymin, ymax)
    row = connection.execute(
        f"""
        select count(*) as total_count
...
def read_prepared_edges_for_nodes(connection, *, dataset_id, layout_version, lod_level, node_ids):
    if not node_ids:
        return ()
    rows = connection.execute(
        """
        ...
        """,
        (dataset_id, layout_version, lod_level, list(sorted(node_ids)), list(sorted(node_ids))),
    ).fetchall()
...
def read_node_positions_by_ids(connection, *, dataset_id, layout_version, node_ids, max_nodes):
...
def search_nodes(connection_context, *, dataset_id, layout_version, query, limit):
    needle = query.strip()
    if not needle:
        return SearchReadResult(dataset_id=dataset_id, layout_version=layout_version, query="", matches=(), total_count=0)
    with connection_context as connection:
        ...
        for row in rows:
            ...
            score = SEARCH_SCORE_ID_EXACT if lower_id == lowered else SEARCH_SCORE_ID_PREFIX if lower_id.startswith(lowered) else SEARCH_SCORE_ID_SUBSTRING
        ...
        for row in rows:
            ...
            if _is_union_node_id(node_id):
                continue
            matched_value = _first_matching_value(json.loads(row["metadata_json"]), lowered)
            if matched_value is not None:
                record_match(best, node_id=node_id, score=SEARCH_SCORE_METADATA_VALUE, matched_text=f"{node_id} {matched_value}")
        ...
        locations = node_locations(connection, dataset_id=dataset_id, layout_version=layout_version, node_ids=[match.node_id for match in limited])
    return SearchReadResult(
        dataset_id=dataset_id,
        layout_version=layout_version,
        query=needle,
        matches=tuple(replace(match, cluster_id=locations.get(match.node_id, (None, None, None))[0], x=locations.get(match.node_id, (None, None, None))[1], y=locations.get(match.node_id, (None, None, None))[2]) for match in limited),
        total_count=len(ordered),
    )


def record_match(best: dict[str, SearchMatch], *, node_id: str, score: int, matched_text: str) -> None:
    existing = best.get(node_id)
    if existing is None or existing.score < score:
        best[node_id] = SearchMatch(node_id=node_id, score=score, matched_text=matched_text)
